...
const simpleGraph = {
	draw: function ( ImageData, graphDataArray, graphBoundaries, isConnected, isEquiscaled )
	{
	...
	// 
		...
		let horisontalScale = horSpan / WIDTH;
		let verticalScale = vertSpan / HEIGHT;
		...
		// paint background
		canvas.fill( ImageData, backgroundColor );
		...
		if (isConnected)
		{
			for( let n = 1; n < graphDataArray.length; n++ )
			{
				...
				canvas.putLine( ImageData, lineColor, ends );
			}
		}
		// plot dots
		for( let n = 0; n < graphDataArray.length; n++ )
		{
			...
			canvas.putDot( ImageData, dotColor, x, y );
		}

	},


	addAxes: function (ImageData, graphBoundaries, colors, scales)
	{
	...
	//	find appropriate scale
		...
		while ( gridLineX < horSpanReal )
		{
			...
			canvas.putLine(ImageData, colors.secondary, gridLineEnds);
	...
	//  ticks are small lines (10px long) crossing main axes, they are 10 times more frequent than grid lines
		...
		while ( tickX < horSpanReal )
		{
			...
			canvas.putLine(ImageData, colors.main, gridLineEnds);
	...
	//  main axes
		...
		canvas.putLine(ImageData, colors.main, vertAxisEnds);
		canvas.putLine(ImageData, colors.main, horAxisEnds);
	}
